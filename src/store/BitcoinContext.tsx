...
export const BitcoinContextProvider = ({
  ...
  // const initBitcoinSession = useCallback(
  //   () =>
  //     new BitcoinSession(client, deviceId, () => connect(deviceId, password)),
  //   [client, connect, deviceId, password]
  // );
  ...
  const fetchBtcData = async (isRecursion = false) => {
    ...
    session.fetchBtcAddresses((err, newAddrCounts) => {
      ...
      session.fetchBtcStateData(opts, (err) => {
        ...
        // @ts-ignore - Recurse such that we exit if there are no new addresses
        fetchBtcData(true);
      });
    });
  };
