...
import { Avatar, Button, Card, Divider, List, Row, Statistic, Tag } from "antd";
...
class WalletPage extends React.Component<any, any> {
  ...
  renderLastUpdatedTag() {
    ...
    const lastUpdated = this.context.session.lastFetchedBtcData;
    if (!lastUpdated) {
    ...
    return <Tag color={color}>{`${elapsed} ${timeType} ago`}</Tag>;
  }

  renderList() {
    const txs = {
      confirmed: this.context.session.getBtcTxs(),
      pending: this.context.session.getBtcTxs(false),
    };
    return (
      <div>
  ...
  renderHeader() {
    ...
    const btcPrice = this.context.session.btcPrice;
    return (
      <div>
  ...
  renderPages() {
    const page = this.context.session.getPage();
    const txs = this.context.session.getBtcTxs();
    return (
      <center style={{ margin: "20px 0 0 0" }}>
        {page > 1 ? (
          <Button
            onClick={() => {
              this.props.pageTurnCb(page - 1);
            }}
          >
            <CaretLeftOutlined />
          </Button>
      ...
      <Card title={`BTC Wallet`} bordered={true}>
        <center>
          ...
          <Button
            ...
            ghost
            onClick={() => {
              this.context.fetchBtcData();
            }}
            loading={this.context.isLoadingBtcData}
            disabled={this.context.isLoadingBtcData}
          >
            Start
          </Button>
        </center>
  ...
  render() {
    const content = (
      <center>{this.context.session ? this.renderContent() : null}</center>
    );
