...
class SendPage extends React.Component<any, any> {
  static contextType = BitcoinContext;
    ...
    fetch("https://blockstream.info/api/fee-estimates")
      ...
      .then((resp) => {
        ...
        if (this.context.session) {
          this.context.session.getBtcWalletData();
        }
      })
      .catch((err) => {
  ...
  checkValue(val) {
    ...
    const balance =
      this.context.session.getBtcBalance() / constants.SATS_TO_BTC;
  ...
  //========================================================
  buildBtcRequest() {
    const req = buildBtcTxReq(
      this.state.recipient,
      this.state.value,
      this.context.session.btcUtxos,
      this.context.session.addresses["BTC"],
      this.context.session.addresses["BTC_CHANGE"],
    ...
    if (req.error) {
      ...
      this.setState({
        error: "Invalid response when building BTC transaction request. ",
      });
      return null;
    }
    return req;
  ...
  renderBanner() {
    if (this.state.error) {
      ...
      const desc = this.context.isMobile ? (
        ...
        <p>
          Your transaction was signed and broadcast successfully. Please return
          to your History tab and refresh to see status.
        </p>
      );
      return <Alert type="success" message="Success" description={desc} />;
    } else {
      return;
    }
  }
  ...
  calculateMaxValue() {
    const balance = this.context.session.getBtcBalance();
    const utxos = this.context.session.getBtcUtxos();
  ...
  renderSubmitButton() {
    ...
    const isValidReq =
      ...
      (allChecks.BTC.full(this.state) ||
        this.state.ensResolvedAddress !== null);

    if (this.state.isLoading) {
      return (
        <Button type="primary" style={{ margin: "30px 0 0 0" }} loading>
          Waiting...
        </Button>
      );
    } else if (isValidReq) {
      ...
      return (
        <Button type="primary" style={{ margin: "30px 0 0 0" }} disabled>
          Send
        </Button>
      );
    }
  ...
  renderBalance() {
    let balance = this.context.session.getBtcBalance() / constants.SATS_TO_BTC;
    return (
  ...
  renderCard() {
    const hasAddressesSlot = this.context.session.addresses.BTC;
    const hasAddresses = hasAddressesSlot
      ? this.context.session.addresses.BTC.length > 0
      : false;
    if (hasAddresses) {
      return (
        <div>
          {this.renderBalance()}
          <div>{this.renderRecipientLabel()}</div>
          <div style={{ margin: "20px 0 0 0" }}>{this.renderValueLabel()}</div>
          <div style={{ margin: "20px 0 0 0" }}>{this.renderExtra()}</div>
      ...
      return (
        <div>
          <p>
            No addresses found from your Lattice. If you have your wallet setup,
            please try logging out and reconnecting.
          </p>
